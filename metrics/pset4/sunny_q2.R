...
event_tstat <- function(N, rho, theta = 1){
  ...
  wald_boot <- map_dbl(1:200, function(b) wild_bootstrap(x, y, cl))
...
tests_output <-
  tests %>%
  mutate(error = factor(error, levels = c("homo", "robust", "cluster"))) %>%
  arrange(error) %>%
